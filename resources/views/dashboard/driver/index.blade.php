                ...
                <div class="card-default">
                    <div class="wrapper d-flex align-items-center justify-content-between gap-2">
                        <form class="form w-100" method="GET">
                            <input type="search" class="input w-100" id="search" placeholder="Search driver.."
                                autocomplete="off" name="search" value="{{ $search != null ? $search : '' }}">
                        </form>
                        <a href="{{ route('driver.create') }}" class="button-primary-small d-none d-md-inline-block">Add New
                            Driver</a>
                    </div>
                    <div class="wrapper-table mt-4">
                        <table class="table" style="width:100%">
                            ...
                            <tbody>
                                @if ($drivers->count() == 0)
                                    <td colspan="6">Data driver not found!</td>
                                @else
                                    @foreach ($drivers as $driver)
                                        <tr>
                                            ...
                                            <td>{{ Str::limit($driver->address, '40') }}</td>
                                            <td>{{ $driver->status === 1 ? 'Active' : 'Unactive' }}</td>
...
@push('child-script')
    <script>
        $(document).on('click', '[data-bs-target="#deleteModal"]', function() {
            let id = $(this).data('id');
            $('#buttonDeleteDriver').attr('action', '/driver/' + id);
        });
    </script>
@endpush
